...
require_once dirname(__FILE__) . '/../helpers/messages.inc';
    ...
    if(!empty($_POST))
    {
        ...
        // their username.
        $query = "
            SELECT jos_users.id, jos_users.name, jos_users.username, jos_users.usertype ,jos_users.password, email, telephone1, website, registerDate, lastvisitDate
            FROM jos_users
        ...
        if($login_ok)
        {
			...
			$query = "INSERT INTO log(user, event, param) VALUES (:username,'Enters','".$ip."')";
			$stmt = $db->prepare($query);
            $result = $stmt->execute($query_params);
